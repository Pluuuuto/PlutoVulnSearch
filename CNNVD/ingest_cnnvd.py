"""CNNVD data ingestion script (renamed from main.py)."""
...
def run():
    # 数据目录位于当前模块同级: CNNVD/data
    data_dir = pathlib.Path(__file__).resolve().parent / 'test'
    if not data_dir.is_dir():
        logger.warning("数据目录不存在: %s (跳过 CNNVD 导入)", data_dir)
    ...
    conn.close()
    logger.info(f"✅ CNNVD 导入完成，总计 {all_success}")

    if all_skipped_logs:
        with open('log/import_skipped.log', 'w', encoding='utf-8') as f:
            for log in all_skipped_logs:
                f.write(f"[{log['file']}] vuln_id={log['vuln_id']} 原因:{log['reason']}\n")
    if all_failed_logs:
        with open('log/import_errors.log', 'w', encoding='utf-8') as f:
            for log in all_failed_logs:
                vid = log.get('vuln_id') or log.get('cve_id') or '未知'
                f.write(f"[{log.get('file','?')}] vuln_id={vid} 错误:{log.get('error','?')}\n")
